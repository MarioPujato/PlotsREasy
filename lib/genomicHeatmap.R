...
expGroups  = factor( unlist( strsplit( input$groupsGHM, split = "," )))

if( numSamples != length(expGroups) ){
	write( paste("[GHM |",curDate,"|",sessionId,"] ERROR: Groups do not match number of samples"), file=stderr() )
	pca = ggplot() +
	theme_void() +
	...
	write( paste("[GHM |",curDate,"|",sessionId,"] Running DESeq2"), file=stderr() )

	# Running DESeq2 and get normalized values
	bigSampleLimit = 1
	if( length(expGroups)>=bigSampleLimit ){
		write( paste("[GHM | ",curDate,"|",sessionId," ] Using vst transformation", sep=''), file=stderr() )
		dataRle = varianceStabilizingTransformation( ddsMatrix, blind = TRUE )
	}else{
		write( paste("[GHM | ",curDate,"|",sessionId," ] Using rlog transformation", sep=''), file=stderr() )
	...
	write( paste("[runGHM |",curDate,"|",sessionId,"] DONE"), file=stderr() )

	percPC1 = summary$importance[2,1]
	percPC2 = summary$importance[2,2]

	write( paste("[runGHM |",curDate,"|",sessionId,"] Generating GHM plot"), file=stderr() )
	...
	# Main title
	mTitle     = gsub( ":n:", "\n", input$titleGHM )
	mTitleSize = input$titleSizeGHM
	legendSize = input$legendSizeGHM
	labelSize  = input$labelSizeGHM
	scaleSize  = input$scaleSizeGHM
	textSize   = input$textSizeGHM/3
	lineWidth  = 4
